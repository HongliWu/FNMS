...
def nms_result_2(data_1, data_2, nms_thresh=0.05, fusion_thresh=0.9):
    gt_boxes_1 = data_1['gt_boxes']
    gt_category_1 = data_1['gt_category']
    boxes_1 = data_1['boxes']
    scores_1 = data_1['scores']
    ...
    for batch_id in range(batch_num):
        ...
        for image_id in range(batch_size):
            ...
            if scores_image.sum() == 0:
                ...
                boxes_fusioned, scores_fusioned, labels_fusioned = nms(boxes_image, scores_image, labels_image, iou_thr=nms_thresh)  # , sigma=nms_thresh)
...
if __name__  == "__main__":
    ...
    # single model
    before_nms_se_nmsed = nms_result(before_nms_se, soft_nms=False, nms_thresh=0.05)
    check_metric(before_nms_se_nmsed, thresh_strage='cvprw')

    before_nms_res2net_nmsed = nms_result(before_nms_res2net, soft_nms=False, nms_thresh=0.05)
    check_metric(before_nms_res2net_nmsed, thresh_strage='cvprw')
    print('single model ↑')

    data_fusioned = nms_result_2(after_nms_se, after_nms_res2net)
